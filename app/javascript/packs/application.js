...
const validateFormFields = (
  ...
  passwordConfirmation.addEventListener("input", (event) => {
    if (passwordInput.value === event.target.value) {
      document.getElementById("password-confirmation-error").style.display =
        "none";
    } else {
      document.getElementById("password-confirmation-error").style.display =
        "block";
...
document.addEventListener("turbolinks:load", function () {
  initSweetalert(
    "#alert-checkin",
    {
      ...
      text:
        "We will inform the property manager that you arrived at the property.",
      icon: "success",
...
const validateForm = () => {
  if (document.querySelector("#new_property")) {
    document
      .querySelector("#new_property .btn-dark")
      .classList.add("btn-dark-disabled");
    document.querySelector("#new_property").addEventListener("input", () => {
      let checked = [
        ...document.getElementsByName("property[property_type]"),
      ].some((c) => c.checked);
      ...
      if (checked && sizeValid && addressValid) {
        ...
        document
          .querySelector("#new_property .btn-dark")
          .classList.remove("btn-dark-disabled");
      } else {
...
document.addEventListener("turbolinks:load", () => {
  if (window.location.href.indexOf("wallet") > 0) {
    //0 is the index of the string wallet within my URL
